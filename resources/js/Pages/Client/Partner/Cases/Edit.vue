    ...
    <div class="space-y-6">
        ...
        <div class="bg-white shadow-sm rounded-lg p-6">
            <form @submit.prevent="submitForm" novalidate>
                ...
                <!-- Action Buttons -->
                <div class="mt-8 flex flex-col md:flex-row md:justify-between gap-3 md:gap-4">
                    <button
                        v-if="caseData.status !== 'archived' && !processing"
                        @click="archiveCase"
                        class="w-full md:w-auto px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none"
                    >
                        Архивировать кейс
                    </button>

                    <div class="flex flex-col md:flex-row gap-3 md:gap-4 md:ml-auto">
                        <Link
                            :href="route('partner.cases.show', { case: caseData.id })"
                            class="w-full md:w-auto px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none text-center"
                        >
                            Отмена
                        </Link>
                        <button
                            type="submit"
                            :disabled="processing"
                            class="w-full md:w-auto inline-flex justify-center py-2 px-6 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none disabled:opacity-50"
                        >
